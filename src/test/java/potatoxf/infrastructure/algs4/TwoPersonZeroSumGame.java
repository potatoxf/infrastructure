...
public class TwoPersonZeroSumGame {
    ...
    private static void test(String description, double[][] payoff) {
        ...
        for (int i = 0; i < m - 1; i++)
            Std.printf("%8.4f, ", y[i]);
        Std.printf("%8.4f]\n", y[m - 1]);
        Std.println("value =  " + zerosum.value());
    ...
    private boolean isPrimalFeasible() {
        ...
        for (int j = 0; j < n; j++) {
            if (x[j] < 0) {
                Std.println("row vector not a probability distribution");
                Std.printf("    x[%d] = %f\n", j, x[j]);
                return false;
            }
            sum += x[j];
        ...
        if (Math.abs(sum - 1.0) > EPSILON) {
            Std.println("row vector x[] is not a probability distribution");
            Std.println("    sum = " + sum);
            return false;
        }
        return true;
    ...
    private boolean isDualFeasible() {
        ...
        for (int i = 0; i < m; i++) {
            if (y[i] < 0) {
                Std.println("column vector y[] is not a probability distribution");
                Std.printf("    y[%d] = %f\n", i, y[i]);
                return false;
            }
            sum += y[i];
        ...
        if (Math.abs(sum - 1.0) > EPSILON) {
            Std.println("column vector not a probability distribution");
            Std.println("    sum = " + sum);
            return false;
        }
        return true;
    ...
    private boolean isNashEquilibrium(double[][] payoff) {
        ...
        if (Math.abs(opt1 - value) > EPSILON) {
            Std.println("Optimal value = " + value);
            Std.println("Optimal best response for column player = " + opt1);
            return false;
        }
        ...
        if (Math.abs(opt2 - value) > EPSILON) {
            Std.println("Optimal value = " + value);
            Std.println("Optimal best response for row player = " + opt2);
            return false;
        }
