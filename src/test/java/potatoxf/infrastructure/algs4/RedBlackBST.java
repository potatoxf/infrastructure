...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; !Std.isEmpty(); i++) {
            String key = Std.readString();
            st.put(key, i);
        }
        for (String s : st.keys())
            Std.println(s + " " + st.get(s));
        Std.println();
    }
    ...
    private boolean check() {
        if (!isBST()) Std.println("Not in symmetric order");
        if (!isSizeConsistent()) Std.println("Subtree counts not consistent");
        if (!isRankConsistent()) Std.println("Ranks not consistent");
        if (!is23()) Std.println("Not a 2-3 tree");
        if (!isBalanced()) Std.println("Not balanced");
