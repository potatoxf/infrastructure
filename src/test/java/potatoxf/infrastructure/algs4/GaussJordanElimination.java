...
public class GaussJordanElimination {
    ...
    private static void test(String name, double[][] A, double[] b) {
        ...
        if (gaussian.isFeasible()) {
            Std.println("Solution to Ax = b");
            double[] x = gaussian.primal();
            for (int i = 0; i < x.length; i++) {
                Std.printf("%10.6f\n", x[i]);
            }
        } else {
            Std.println("Certificate of infeasibility");
        ...
        if (isFeasible()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (Math.abs(sum - b[i]) > EPSILON) {
                    Std.println("not feasible");
                    Std.printf("b[%d] = %8.3f, sum = %8.3f\n", i, b[i], sum);
                    return false;
                }
            }
            return true;
        ...
        // or that yA = 0, yb != 0
            ...
            for (int j = 0; j < n; j++) {
                ...
                if (Math.abs(sum) > EPSILON) {
                    Std.println("invalid certificate of infeasibility");
                    Std.printf("sum = %8.3f\n", sum);
                    return false;
                }
            }
