...
public class PatriciaSET implements Iterable<String> {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            Std.printf("Creating dataset (%d items)...\n", limitItem);
            for (int i = 0; i < limitItem; i++)
                a[i] = Integer.toString(i, 16);

            Std.printf("Shuffling...\n");
            StdRandom.shuffle(a);

            Std.printf("Adding (%d items)...\n", limitItem);
            for (int i = 0; i < limitItem; i++)
                set.add(a[i]);

            int countItems = 0;
            Std.printf("Iterating...\n");
            for (String key : set) countItems++;
            Std.printf("%d items iterated\n", countItems);
            ...
            Std.printf("%d items found and %d (deleted) items missing\n",
            ...
            Std.printf("Deleting the rest (%d items)...\n",
            ...
            countPass++;
            if (ok) Std.printf("PASS %d TESTS SUCCEEDED\n", countPass);
            else Std.printf("PASS %d TESTS FAILED\n", countPass);
