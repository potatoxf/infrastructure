 ...
 *  Dependencies: DijkstraSP.java Edge.java
 *
...
public class AssignmentProblem {
    ...
    private void augment() {
        ...
        for (int i = 0; i < n; i++) {
            if (xy[i] == UNMATCHED)
                G.addEdge(new Graph.Edge(s, i, 0.0));
        }
        for (int j = 0; j < n; j++) {
            if (yx[j] == UNMATCHED)
                G.addEdge(new Graph.Edge(n + j, t, py[j]));
        }
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (xy[i] == j) G.addEdge(new Graph.Edge(n + j, i, 0.0));
                else G.addEdge(new Graph.Edge(i, n + j, reducedCost(i, j)));
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (reducedCost(i, j) < 0) {
                    Std.println("Dual variables are not feasible");
                    return false;
                }
            }
        }
    ...
    private boolean isComplementarySlack() {
        ...
        for (int i = 0; i < n; i++) {
            if ((xy[i] != UNMATCHED) && (reducedCost(i, xy[i]) != 0)) {
                Std.println("Primal and dual variables are not complementary slack");
                return false;
            }
        }
        return true;
    ...
    private boolean isPerfectMatching() {
        ...
        for (int i = 0; i < n; i++) {
            if (perm[xy[i]]) {
                Std.println("Not a perfect matching");
                return false;
            }
            perm[xy[i]] = true;
        }
        ...
        for (int j = 0; j < n; j++) {
            if (xy[yx[j]] != j) {
                Std.println("xy[] and yx[] are not inverses");
                return false;
            }
        }
        for (int i = 0; i < n; i++) {
