...
public class HopcroftKarp {
    ...
    public static void main(String[] args) {
        ...
        if (G.vertexCount() < 1000) Std.println(G);

        HopcroftKarp matching = new HopcroftKarp(G);

        // print maximum matching
        Std.printf("Number of edges in max matching        = %d\n", matching.size());
        Std.printf("Number of vertices in min vertex cover = %d\n", matching.size());
        Std.printf("Graph has a perfect matching           = %b\n", matching.isPerfect());
        Std.println();

        if (G.vertexCount() >= 1000) return;
