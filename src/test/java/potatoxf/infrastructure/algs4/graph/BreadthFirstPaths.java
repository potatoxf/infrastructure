...
public class BreadthFirstPaths {
    ...
    public static void main(String[] args) {
        ...
        for (int v = 0; v < G.vertexCount(); v++) {
            if (bfs.hasPathTo(v)) {
                Std.printf("%d to %d (%d):  ", s, v, bfs.distTo(v));
                ...
                Std.printf("%d to %d (-):  not connected\n", s, v);
            }
    ...
    private boolean check(Graph G, int s) {
        ...
        if (distTo[s] != 0) {
            Std.println("distance of source " + s + " to itself = " + distTo[s]);
            return false;
        }
        ...
        for (int v = 0; v < G.vertexCount(); v++) {
            for (int w : G.adjacencyVertex(v)) {
                if (hasPathTo(v) != hasPathTo(w)) {
                    Std.println("edge " + v + "-" + w);
                    Std.println("hasPathTo(" + v + ") = " + hasPathTo(v));
                    Std.println("hasPathTo(" + w + ") = " + hasPathTo(w));
                    return false;
                }
                if (hasPathTo(v) && (distTo[w] > distTo[v] + 1)) {
                    Std.println("edge " + v + "-" + w);
                    Std.println("distTo[" + v + "] = " + distTo[v]);
                    Std.println("distTo[" + w + "] = " + distTo[w]);
                    return false;
                }
            }
        }
        ...
        for (int w = 0; w < G.vertexCount(); w++) {
            ...
            if (distTo[w] != distTo[v] + 1) {
                Std.println("shortest path edge " + v + "-" + w);
                Std.println("distTo[" + v + "] = " + distTo[v]);
                Std.println("distTo[" + w + "] = " + distTo[w]);
                return false;
            }
