...
public class BipartiteX {
    ...
    public BipartiteX(Graph G) {
        isBipartite = true;
        color = new boolean[G.vertexCount()];
        marked = new boolean[G.vertexCount()];
        edgeTo = new int[G.vertexCount()];

        for (int v = 0; v < G.vertexCount() && isBipartite; v++) {
            if (!marked[v]) {
                bfs(G, v);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        if (b.isBipartite()) {
            Std.println("Graph is bipartite");
            for (int v = 0; v < G.vertexCount(); v++) {
                Std.println(v + ": " + b.color(v));
            }
        } else {
            Std.print("Graph has an odd-length cycle: ");
