...
public class LazyPrimMST {
    ...
    private final Queue<Graph.Edge> mst;     // edges in the MST
    private final boolean[] marked;    // marked[v] = true if v on tree
    private final MinPQ<Graph.Edge> pq;      // edges with one endpoint in tree
    ...
    public LazyPrimMST(Graph G) {
        mst = new Queue<Graph.Edge>();
        pq = new MinPQ<Graph.Edge>();
        marked = new boolean[G.vertexCount()];
        for (int v = 0; v < G.vertexCount(); v++)     // run Prim from all vertices to
    ...
    private void prim(Graph G, int s) {
        ...
        while (!pq.isEmpty()) {                        // better to stop when mst has V-1 edges
            Graph.Edge e = pq.delMin();                      // smallest edge on pq
