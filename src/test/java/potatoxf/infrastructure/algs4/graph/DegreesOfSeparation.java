...
public class DegreesOfSeparation {
    ...
    public static void main(String[] args) {
        ...
        SymbolGraph sg = GraphParser.parseSymbol(filename, delimiter, false);
        if (!sg.hasVertexSymbol(source)) {
            Std.println(source + " not in database.");
            return;
        }

        int s = sg.indexOf(source);
        BreadthFirstPaths bfs = new BreadthFirstPaths(sg, s);

        while (!Std.isEmpty()) {
            String sink = Std.readLine();
            if (sg.hasVertexSymbol(sink)) {
