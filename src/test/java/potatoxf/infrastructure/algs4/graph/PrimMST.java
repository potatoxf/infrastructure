...
public class PrimMST {
    ...
    private final Graph.Edge[] edgeTo;        // edgeTo[v] = shortest edge from tree vertex to non-tree vertex
    ...
    public PrimMST(Graph G) {
        ...
        for (int v = 0; v < G.vertexCount(); v++)      // run from each vertex to find
    ...
    public Iterable<Graph.Edge> edges() {
        Queue<Graph.Edge> mst = new Queue<Graph.Edge>();
        for (int v = 0; v < edgeTo.length; v++) {
            Graph.Edge e = edgeTo[v];
            if (e != null) {
                mst.enqueue(e);
            }
        }
