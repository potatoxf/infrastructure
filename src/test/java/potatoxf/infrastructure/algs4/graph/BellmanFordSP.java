...
public class BellmanFordSP {
    ...
    private Iterable<Graph.Edge> cycle;  // negative cycle (or null if no such cycle)
     ...
     */
    public BellmanFordSP(Graph G, int s) {
        distTo = new double[G.vertexCount()];
        edgeTo = new Graph.Edge[G.vertexCount()];
        onQueue = new boolean[G.vertexCount()];
    ...
    public static void main(String[] args) {
        ...
        if (sp.hasNegativeCycle()) {
            for (Graph.Edge e : sp.negativeCycle())
                Std.println(e);
        }
        ...
        // print shortest paths
            ...
            for (int v = 0; v < G.vertexCount(); v++) {
                if (sp.hasPathTo(v)) {
                    Std.printf("%d to %d (%5.2f)  ", s, v, sp.distTo(v));
                    for (Graph.Edge e : sp.pathTo(v)) {
                        Std.print(e + "   ");
                    }
                    Std.println();
                } else {
    ...
    private boolean check(Graph G, int s) {
        ...
        if (hasNegativeCycle()) {
            double weight = 0.0;
            for (Graph.Edge e : negativeCycle()) {
                weight += e.weight();
            }
            if (weight >= 0.0) {
        ...
        Std.println("Satisfies optimality conditions");
        Std.println();
        return true;
    }
